...
def segment_images(image_stack):
    ...
    shift_vectors = [skimage.registration.phase_cross_correlation(np.log10(1+image_channel_sum[0]), np.log10(1+image_channel_sum[i]))[0] for i in range(1,len(image_stack))]
    ...
    segmentation = skimage.segmentation.watershed(-image_cn, watershed_seeds, mask = rough_seg_mask)
