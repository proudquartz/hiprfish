...
def remove_epithepial_objects_and_debris(image_seg, image_identification, adjacency_seg, epithelial_region, taxon_lookup, cell_info, sample):
    epithelia = image_seg*epithelial_region
    image_identification_filtered = image_identification.copy()
    image_identification_filtered[epithelia > 0] = [0.5,0.5,0.5]
    epithelial_labels = np.delete(np.unique(epithelia), 0)
    for i in range(cell_info.shape[0]):
        ...
        if (cell_label in epithelial_labels):
          cell_info.loc[i, 'type'] = 'epithelia'
...
def analyze_gut_diversity_ciprofloxacin(image_tab_gut_filename, probe_design_filename):
    taxa_barcode_sciname = get_taxa_barcode_sciname(probe_design_filename)
    taxa_barcode_sciname = get_taxa_barcode_sciname_lineage(taxa_barcode_sciname)
    # Shannon diversity time series
    ...
    for i in range(image_tab.shape[0]):
        ...
        image_tab.loc[i,'SHANNON_DIVERSITY'] = shannon_diversity
        image_tab.loc[i,'CELL_NUMBER'] = cell_info.shape[0]
        cell_info_cipro_all.append(cell_info)

    cell_info_cipro_all = pd.concat(cell_info_cipro_all)
    ...
    theme_color = 'black'
    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(3.125), cm_to_inches(2.75))
    ...
    plt.bar(0, control_bacteroides_abundance, bottom = control_others_abundance + control_firmicutes_abundance, width = 0.5, color = (0,0.25,1), label = 'Bacteroidetes')
    plt.bar(0, control_firmicutes_abundance, bottom = control_others_abundance, width = 0.5, color = (1,0.25,0), label = 'Firmicutes')
    # plt.bar(0, 0, width = 0.5, color=(0, 0, 0, 0), label = ' ')
    ...
    plt.bar(1, antibiotics_firmicutes_abundance, bottom = antibiotics_others_abundance, width = 0.5, color = (1,0.25,0))
    plt.bar(1, antibiotics_bacteroides_abundance, bottom = antibiotics_others_abundance + antibiotics_firmicutes_abundance, width = 0.5, color = (0,0.25,1))
    plt.xticks([0,1], ['control', 'cipro'])
    plt.ylabel('Abundance', fontsize = 6, color = theme_color, labelpad = 5)
    plt.tick_params(direction = 'in', length = 2, pad = 2, colors = theme_color, width = 0.5, labelsize = 6)
    l = plt.legend(frameon = False, fontsize = 5, bbox_to_anchor = (0.17,0.58), handlelength = 1, handletextpad = 1, labelspacing = 0.25)
    # l.get_title().set_fontsize('6')
    l._legend_box.align = 'left'
    plt.ylim(0,1.625)
    plt.xlim(-0.5, 1.5)
    plt.subplots_adjust(left = 0.26, bottom = 0.14, right = 0.98, top = 0.98)
    plt.savefig('{}/gut_phylum_abundance_treatment_ciprofloxacin.pdf'.format(data_folder), dpi = 300)
    plt.close()

    # phylum composition presentation
    theme_color = 'white'
    font_size = 12
    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(7.5), cm_to_inches(7.5))
    antibiotics_bacteroides_abundance = barcode_abundance_antibiotics.loc[barcode_abundance_antibiotics.taxid.values == 976, 'abundance'].values[0]
    antibiotics_firmicutes_abundance = barcode_abundance_antibiotics.loc[barcode_abundance_antibiotics.taxid.values == 1239, 'abundance'].values[0]
    antibiotics_others_abundance = 1 - antibiotics_bacteroides_abundance - antibiotics_firmicutes_abundance
    control_bacteroides_abundance = barcode_abundance_control.loc[barcode_abundance_control.taxid.values == 976, 'abundance'].values[0]
    ...
    plt.bar(1, antibiotics_firmicutes_abundance, bottom = antibiotics_others_abundance, width = 0.5, color = (1,0.25,0))
    plt.bar(1, antibiotics_bacteroides_abundance, bottom = antibiotics_others_abundance + antibiotics_firmicutes_abundance, width = 0.5, color = (0,0.25,1))
    plt.xticks([0,1], ['Control', 'Ciprofloxacin'])
    ...
    l = plt.legend(frameon = False, fontsize = font_size, bbox_to_anchor = (0.25,0.64), handlelength = 1, handletextpad = 1, labelspacing = 0.5)
    # l.get_title().set_fontsize('6')
    l._legend_box.align = 'left'
    ...
    plt.ylim(0,1.5)
    plt.xlim(-0.5, 1.6)
    plt.subplots_adjust(left = 0.22, bottom = 0.1, right = 0.98, top = 0.98)
    plt.savefig('{}/gut_phylum_abundance_treatment_ciprofloxacin_presentation.pdf'.format(data_folder), dpi = 300, transparent = True)
    plt.close()

    # Shannon diversity
    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(2.875), cm_to_inches(2.75))
    image_tab_nc = image_tab.loc[image_tab.CELL_NUMBER.values > 500, :]
    ...
    flierprops = dict(marker='.', markerfacecolor='black', markersize=8, linestyle='none', markeredgewidth = 0)
    medianprops = dict(linestyle='-', linewidth=2, color=(0,0,0))
    bp = plt.boxplot(shannon_diversity_list, positions = np.arange(2), widths = 0.5, patch_artist = True, flierprops = flierprops, medianprops = medianprops)
    for b in bp['boxes']:
        b.set_facecolor((0.6,0.6,0.6))

    plt.tick_params(direction = 'in', length = 2, labelsize = 6, pad = 2)
    plt.xticks(np.arange(2), ['control', 'cipro'])
    plt.ylabel('Shannon Diversity', fontsize = 6, color = theme_color, labelpad = 0)
    plt.subplots_adjust(left = 0.28, bottom = 0.14, right = 0.98, top = 0.98)
    plt.savefig('{}/gut_shannon_diversity_treatment_ciprofloxacin.pdf'.format(data_folder), dpi = 300)

    # Shannon diversity presentation
    theme_color = 'white'
    font_size = 12
    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(7.5), cm_to_inches(7.5))
    ...
    treatments = image_tab_nc.TREATMENT.drop_duplicates().sort_values(ascending = False)
    shannon_diversity_list = [image_tab_nc.loc[image_tab_nc.TREATMENT.values == t, 'SHANNON_DIVERSITY'].values for t in treatments]
    flierprops = dict(marker='.', markerfacecolor='white', markersize=8, linestyle='none', markeredgewidth = 0)
    medianprops = dict(linestyle='-', linewidth=2, color=(1,1,1))
    bp = plt.boxplot(shannon_diversity_list, positions = np.arange(2), widths = 0.5, patch_artist = True, flierprops = flierprops, medianprops = medianprops)
    for b in bp['boxes']:
        b.set_facecolor((0.6,0.6,0.6))
        b.set_edgecolor((1,1,1))
    ...
    plt.axes().spines['top'].set_color(theme_color)
    plt.axes().spines['right'].set_color(theme_color)
    plt.xticks(np.arange(2), ['Control', 'Ciprofloxacin'])
    plt.ylabel('Shannon Diversity', fontsize = font_size, color = theme_color)
    plt.subplots_adjust(left = 0.22, bottom = 0.1, right = 0.98, top = 0.98)
    plt.savefig('{}/gut_shannon_diversity_treatment_ciprofloxacin_presentation.pdf'.format(data_folder), dpi = 300, transparent = True)
    ...
    # beta diversity patch size
    np.save('{}/gut_cipro_beta_diversity_patch_treatment_source_data.npy'.format(data_folder), beta_diversity_patch_treatment)
    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(3.125), cm_to_inches(2.75))
    ...
    plt.xticks(np.arange(0, 2.2*2, 2.2), ['control', 'cipro'])
    plt.tick_params(direction = 'in', length = 2, labelsize = 6, width = 0.5, colors = theme_color, pad = 2)
    plt.ylabel(r'$\beta$ Diversity', fontsize = 6, color = theme_color, labelpad = 0)
    plt.ylim(0.9,1.96)
    plt.xlim(-1.5, 4)
    cbaxes = fig.add_axes([0.5, 0.9, 0.3, 0.04])
    norm = Normalize(vmin = 10, vmax = 200)
    cb = plt.colorbar(cm.ScalarMappable(norm=norm, cmap=cmap), cax = cbaxes, orientation = 'horizontal')
    cbaxes.tick_params(direction = 'in', length = 2, labelsize = 5, width = 0.5, color = theme_color)
    cb.set_ticks([10,100,200])
    cb.set_label(r'$N_{cells}$', fontsize = 6, color = theme_color, labelpad = 1)
    plt.subplots_adjust(left = 0.26, bottom = 0.2, right = 0.98, top = 0.98)
    plt.savefig('{}/gut_beta_diversity_patch_size_treatment_ciprofloxacin.pdf'.format(data_folder), dpi = 300)
    plt.close()

    # beta diversity patch size presentation
    theme_color = 'white'
    font_size = 12
    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(7.5), cm_to_inches(7.5))
    ...
    for i in np.arange(19):
        bp = plt.boxplot(beta_diversity_patch_treatment[i], positions = np.arange(0, 2.2*2, 2.2) - 1 + 0.1*i, patch_artist = True, flierprops = flierprops,
                    widths = 0.2, showcaps = False, showfliers = False, meanline = False, medianprops = medianprops)
        for b in bp['boxes']:
            b.set_facecolor(cmap(i/20))
            b.set_linewidth(0.5)
            b.set_edgecolor((0.4,0.4,0.4))
    ...
    plt.xticks(np.arange(0, 2.2*2, 2.2), ['control', 'cipro'])
    plt.tick_params(direction = 'in', length = 2, labelsize = font_size, colors = theme_color)
    plt.ylabel(r'$\beta$ Diversity', fontsize = font_size, color = theme_color)
    plt.ylim(0.9,1.96)
    plt.xlim(-1.5, 4)
    plt.axes().spines['left'].set_color(theme_color)
    plt.axes().spines['bottom'].set_color(theme_color)
    ...
    norm = Normalize(vmin = 10, vmax = 200)
    cb = plt.colorbar(cm.ScalarMappable(norm=norm, cmap=cmap), cax = cbaxes, orientation = 'horizontal')
    cbaxes.tick_params(direction = 'in', length = 2, labelsize = font_size, colors = theme_color)
    cb.set_ticks([10,100,200])
    cb.set_label(r'$N_{cells}$', fontsize = font_size, color = theme_color)
    plt.subplots_adjust(left = 0.22, bottom = 0.2, right = 0.98, top = 0.98)
    plt.savefig('{}/gut_beta_diversity_patch_size_treatment_ciprofloxacin_presentation.svg'.format(data_folder), dpi = 300, transparent = True)
    plt.close()

    # bray-curtis dissmilarity patch
    nc = 200
    n_pairs = 200
    n_cell = nc
    bray_curtis_list = []
    ...
    # bray-curtis patch size
    np.save('{}/gut_bray_curtis_nc_{}_ciprofloxacin_source_data.npy'.format(data_folder, n_cell), bray_curtis_list)
    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(2.875), cm_to_inches(2.75))
    cmap = cm.get_cmap('tab10')
    color_list = [(0.75,0,0),(0,0,0.75)]
    treatment = image_tab.TREATMENT.drop_duplicates().sort_values(ascending = False)
    label_list = ['control', 'cipro']
    for i in range(2):
        bcd = bray_curtis_list[i]
        bcd_sorted = bcd[bcd[:,0].argsort()]
        bcd_smoothed = scipy.signal.savgol_filter(bcd_sorted[:,1], window_length = 41, polyorder = 3)
    ...
    plt.tick_params(direction = 'in', length = 2, labelsize = 6, pad = 2, width = 0.5)
    plt.legend(frameon = False, fontsize = 6, bbox_to_anchor = (0.25,0.35), handlelength = 0.5)
    plt.xlabel(r'Intra-patch distance [$\mu$m]', fontsize = 6, color = theme_color, labelpad = 0)
    plt.axes().xaxis.set_label_coords(0.3,-0.15)
    plt.ylabel('Bray-Curtis', fontsize = 6, color = theme_color)
    plt.subplots_adjust(left = 0.28, bottom = 0.2, right = 0.98, top = 0.98)
    plt.savefig('{}/gut_bray_curtis_nc_{}_ciprofloxacin.pdf'.format(data_folder, n_cell), dpi = 300)
    plt.close()

    # bray-curtis patch size presentation
    theme_color = 'white'
    font_size = 12
    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(7.5), cm_to_inches(7.5))
    cmap = cm.get_cmap('tab10')
    color_list = [(1,0.5,0),(0,0.5,1)]
    treatment = image_tab.TREATMENT.drop_duplicates().sort_values(ascending = False)
    ...
    plt.axes().spines['right'].set_color(theme_color)
    plt.subplots_adjust(left = 0.22, bottom = 0.2, right = 0.98, top = 0.98)
    plt.savefig('{}/gut_bray_curtis_nc_{}_ciprofloxacin_presentation.svg'.format(data_folder, n_cell), dpi = 300, transparent = True)
    plt.close()


    return

def analyze_gut_diversity_clindamycin(image_tab_gut_filename):
    ...
    # bacteroidetes to firmicutes ratio 0.635
    # bacteroidetes to firmicutes ratio 0.453

    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(4.25), cm_to_inches(4))
    antibiotics_bacteroidetes_abundance = barcode_abundance_antibiotics.loc[barcode_abundance_antibiotics.taxid.values == 976, 'abundance'].values[0]
    antibiotics_firmicutes_abundance = barcode_abundance_antibiotics.loc[barcode_abundance_antibiotics.taxid.values == 1239, 'abundance'].values[0]
    antibiotics_others_abundance = 1 - antibiotics_bacteroidetes_abundance - antibiotics_firmicutes_abundance
    control_bacteroidetes_abundance = barcode_abundance_control.loc[barcode_abundance_control.taxid.values == 976, 'abundance'].values[0]
    control_firmicutes_abundance = barcode_abundance_control.loc[barcode_abundance_control.taxid.values == 1239, 'abundance'].values[0]
    control_others_abundance = 1 - control_bacteroidetes_abundance - control_firmicutes_abundance
    plt.bar(0, control_bacteroidetes_abundance, bottom = control_others_abundance + control_firmicutes_abundance, width = 0.5, color = (0,0.25,1), label = 'Bacteroides')
    plt.bar(0, control_firmicutes_abundance, bottom = control_others_abundance, width = 0.5, color = (1,0.25,0), label = 'Firmicutes')
    plt.bar(0, control_others_abundance, width = 0.5, color = 'olivedrab', label = 'Other')
    plt.bar(1, antibiotics_others_abundance, width = 0.5, color = 'olivedrab')
    plt.bar(1, antibiotics_firmicutes_abundance, bottom = antibiotics_others_abundance, width = 0.5, color = (1,0.25,0))
    plt.bar(1, antibiotics_bacteroidetes_abundance, bottom = antibiotics_others_abundance + antibiotics_firmicutes_abundance, width = 0.5, color = (0,0.25,1))
    plt.xticks([0,1], ['control', 'clindamycin'], rotation = 0, ha = 'center')
    plt.ylabel('Abundance', fontsize = 6, color = theme_color)
    plt.tick_params(direction = 'in', length = 2, colors = theme_color, labelsize = 6)
    l = plt.legend(frameon = False, fontsize = 6, bbox_to_anchor = (0.25,0.6), title = 'Phylum', handlelength = 1, handletextpad = 1, labelspacing = 0.25)
    l.get_title().set_fontsize('6')
    plt.ylim(0,1.6)
    plt.xlim(-0.5,1.5)
    plt.subplots_adjust(left = 0.22, bottom = 0.16, right = 0.98, top = 0.98)
    ...
    plt.xticks(np.arange(2), ['Control', 'Clindamycin'])
    plt.ylabel('Shannon Diversity', fontsize = 6, color = theme_color)
    plt.subplots_adjust(left = 0.22, bottom = 0.1, right = 0.98, top = 0.98)
    ...
    treatment_list = image_tab.TREATMENT.drop_duplicates().sort_values(ascending = False)
    plt.xticks(np.arange(0, 2.2*2, 2.2), ['Control', 'Clindamycin'])
    plt.tick_params(direction = 'in', length = 2, labelsize = 6, colors = theme_color)
    plt.ylabel(r'$\beta$ Diversity', fontsize = 6, color = theme_color)
    plt.ylim(0.9,2.3)
    plt.xlim(-1.5, 4)
    ...
    cbaxes.tick_params(direction = 'in', length = 2, labelsize=6, color = theme_color)
    cb.set_ticks([10,100,200])
    cb.set_label(r'$N_{cells}$', fontsize = 6, color = theme_color)
    plt.subplots_adjust(left = 0.22, bottom = 0.1, right = 0.98, top = 0.98)
    ...
    plt.tick_params(direction = 'in', length = 2, labelsize = 6)
    # plt.ylim(-0.05, 0.5)
    plt.legend(frameon = False, fontsize = 6, bbox_to_anchor = (0.35,0.38), handlelength = 0.5)
    plt.xlabel(r'Intra-patch distance [$\mu$m]', fontsize = 6, color = theme_color)
    plt.ylabel('Bray-Curtis', fontsize = 6, color = theme_color)
    plt.subplots_adjust(left = 0.22, bottom = 0.2, right = 0.98, top = 0.98)
    plt.savefig('{}/gut_bray_curtis_nc_{}_clindamycin.pdf'.format(data_folder, n_cell), dpi = 300, transparent = True)
...
def compare_oral_gut_bray_curtis():
    ...
    # bray-curtis patch distance oral gut comparison
    theme_color = 'black'
    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(4.5), cm_to_inches(3.5))
    cmap = cm.get_cmap('tab10')
    ...
    # bray-curtis patch distance oral gut comparison presentation
    theme_color = 'white'
    font_size = 12
    fig = plt.figure()
    ...
    for i in range(2):
        ...
        if i == 1:
            ...
            plt.plot(bcd_sorted[:,0]*0.07, bcd_smoothed, color = (0.5,0.5,0.5), label = 'mouse gut', alpha = 0.8)
    ...
    plt.axes().spines['top'].set_color(theme_color)
    plt.axes().spines['right'].set_color(theme_color)
    l = plt.legend(frameon = False, fontsize = 8, bbox_to_anchor = (0.05,0.72,0.8,0.2), ncol = 4, handlelength = 0.5, columnspacing = 0.5, handletextpad = 0.25)
    for t in l.get_texts():
        t.set_color(theme_color)

    plt.xlabel(r'Intra-patch distance [$\mu$m]', fontsize = font_size, color = theme_color, labelpad = 0)
    plt.ylabel('Bray-Curtis', fontsize = font_size, color = theme_color, labelpad = 0)
    plt.subplots_adjust(left = 0.2, bottom = 0.2, right = 0.98, top = 0.98)
...
def measure_spatial_association_vs_abundance_by_treatment(sam_tab, taxon_lookup, data_folder):
    ...
    taxon_lookup = pd.read_csv('/workdir/hs673/Runs/V1/Samples/HIPRFISH_imaging/09_08_2019_DSGN0567/taxon_color_lookup.csv', dtype = {'code':str})
    ...
    for s in range(len(cipro_samples)):
        ...
        amrf_filename = '{}/{}/{}_adjacency_matrix_random_full_{}.npy'.format(data_folder, sample, image_name, n_sim)
    ...
    for s in range(len(control_samples)):
        ...
        amcrf_filename = '{}/{}/{}_adjacency_matrix_control_random_full_{}.npy'.format(data_folder, sample, image_name, n_sim)
        if not os.path.exists(amcrf_filename):
            ...
            np.save('{}/{}/{}_adjacency_matrix_control_random_full_{}.npy'.format(data_folder, sample, image_name, n_sim), adjacency_matrix_control_random_full)
        else:
    ...
    cell_info_cipro_all_df = pd.concat(cell_info_cipro_all)
    ...
    association_fc_comparison = np.average(association_matrix_fold_change_random, axis = 2)/np.average(association_matrix_fold_change_control_random, axis = 2)
    ranking_order = np.argsort(np.sum(np.abs(np.log2(association_fc_comparison)), axis = 0))[::-1]
    ...
    association_fc_comparison_sorted = association_fc_comparison[xx, yy]
    taxa_labels = all_taxa_cell_count.loc[ranking_order, 'sci_name']
    taxa_list_sorted = taxa_list[ranking_order]
    top_n_keep = 20

    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(5.875), cm_to_inches(5.875))
    im = plt.imshow(np.log2(np.average(association_matrix_fold_change_random_sorted, axis = 2))[:top_n_keep,:top_n_keep], cmap = 'coolwarm')
    plt.xticks(np.arange(top_n_keep), taxa_labels[:top_n_keep], rotation = 90)
    plt.yticks(np.arange(top_n_keep), taxa_labels[:top_n_keep])
    plt.axes().yaxis.tick_right()
    plt.axes().xaxis.tick_bottom()
    plt.tick_params(direction = 'in', labelsize = 6, width = 0.5)
    cbaxes = fig.add_axes([0.9, 0.02, 0.02, 0.15])
    cbar = plt.colorbar(im, cax = cbaxes, orientation = 'vertical')
    cbar.ax.tick_params(labelsize = 5, direction = 'in', width = 0.5, length = 3)
    cbar.set_ticks([0, 5])
    cbar.set_label(r'$\log_2$(FC)', color = 'black', fontsize = 5, labelpad = 1)
    cbar.ax.yaxis.tick_left()
    cbar.ax.yaxis.set_label_position('left')
    plt.subplots_adjust(left=0.02, bottom=0.2, right=0.8, top=0.98)
    ...
    im = plt.imshow(np.log2(np.average(association_matrix_fold_change_control_random_sorted, axis = 2))[:top_n_keep,:top_n_keep], cmap = 'coolwarm')
    plt.xticks(np.arange(top_n_keep), taxa_labels[:top_n_keep], rotation = 90)
    plt.yticks(np.arange(top_n_keep), taxa_labels[:top_n_keep])
    plt.axes().yaxis.tick_right()
    plt.axes().xaxis.tick_bottom()
    plt.tick_params(direction = 'in', labelsize = 6)
    cbaxes = fig.add_axes([0.85, 0.05, 0.12, 0.02])
    ...
    # differential spatial association
    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(5.75), cm_to_inches(5.75))
    im = plt.imshow(np.log2(association_fc_comparison_sorted[:top_n_keep,:top_n_keep]), cmap = 'coolwarm')
    plt.xticks(np.arange(top_n_keep), taxa_labels[:top_n_keep], rotation = -90)
    plt.yticks(np.arange(top_n_keep), taxa_labels[:top_n_keep])
    plt.axes().yaxis.tick_right()
    plt.axes().xaxis.tick_bottom()
    plt.tick_params(direction = 'in', labelsize = 5, width = 0.5, length = 2)
    cbaxes = fig.add_axes([0.7, 0.2, 0.2, 0.02])
    cbar = plt.colorbar(im, cax = cbaxes, orientation = 'horizontal')
    cbar.ax.tick_params(labelsize = 5, width = 0.5, direction = 'in', length = 2)
    cbar.set_label(r'$\log_2$(FC)', color = 'black', fontsize = 5, labelpad = 1)
    # cbar.set_ticks([-0.5, 0.5, 1.5,2.0])
    cbar.ax.set_title('differential \n spatial association', fontsize = 5)
    cbar.ax.yaxis.tick_left()
    cbar.ax.yaxis.set_label_position('left')
    plt.subplots_adjust(left=0.01, bottom=0.38, right=0.62, top=0.99)
    plt.savefig('{}/association_fold_change_cipro_random_vs_control_random.pdf'.format(data_folder), dpi = 300, transparent = True)
    plt.close()

    # differential spatial association presentation
    theme_color = 'white'
    font_size = 12
    fig = plt.figure()
    ...
    plt.axes().yaxis.tick_right()
    plt.axes().xaxis.tick_bottom()
    plt.tick_params(direction = 'in', labelsize = font_size, colors = theme_color)
    plt.axes().spines['left'].set_color(theme_color)
    plt.axes().spines['bottom'].set_color(theme_color)
    plt.axes().spines['top'].set_color(theme_color)
    plt.axes().spines['right'].set_color(theme_color)
    cbaxes = fig.add_axes([0.75, 0.15, 0.2, 0.02])
    cbar = plt.colorbar(im, cax = cbaxes, orientation = 'horizontal')
    cbar.ax.tick_params(labelsize = font_size, direction = 'in', length = 3, colors = theme_color)
    cbar.set_label(r'$\log_2$(FC)', color = 'black', fontsize = font_size, labelpad = 1)
    # cbar.set_ticks([-0.5, 0.5, 1.5,2.0])
    cbar.ax.set_title('Differential \n spatial association', fontsize = font_size, color = theme_color)
    cbar.ax.yaxis.tick_left()
    cbar.ax.yaxis.set_label_position('left')
    plt.subplots_adjust(left=0.01, bottom=0.32, right=0.68, top=0.99)
    plt.savefig('{}/association_fold_change_cipro_random_vs_control_random_presentation.svg'.format(data_folder), dpi = 300, transparent = True)
    plt.close()
    ...
    association_nonzero_sorted = association_nonzero[xx,yy]
    for i in range(len(taxa_list_sorted)):
        for j in range(len(taxa_list_sorted)):
            ...
            statistics, pvalue = scipy.stats.ttest_ind(np.log2(association_ij), np.log2(association_control_ij))
            association_fc_significance[i,j] = pvalue
    # fig = plt.figure()
    ...
    n_hypothesis = np.sum(association_nonzero_sorted[:,:])/2
    ...
    for i in range(significant_indices.shape[0]):
        ...
        if barcode_i_index > barcode_j_index:
            ...
            if np.isfinite(np.abs(apfc)) > 0 and aps > 0 and np.abs(np.log2(apfc)) > 1:
                barcode_i = taxa_list_sorted[barcode_i_index]
                barcode_j = taxa_list_sorted[barcode_j_index]
                ...
                if np.log10(apfc) < 0:
                    position_x = np.log2(association_fc[barcode_i_index, barcode_j_index]) - 0.4
                    position_y = -np.log10(association_fc_significance[barcode_i_index, barcode_j_index])
    ...
    # significant differential spatial association
    positions_list_sorted = [positions_list[k] for k in np.argsort(fc_list)[::-1]]
    labels_list_sorted = [labels_list[k] for k in np.argsort(fc_list)[::-1]]
    ha_list_sorted = [ha_list[k] for k in np.argsort(fc_list)[::-1]]
    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(4.25), cm_to_inches(5.5))
    gs = GridSpec(3,1)
    ax = plt.subplot(gs[0:2,0])
    plt.plot(np.log2(np.triu(association_fc*association_nonzero_sorted, k = 1)), -np.log10(np.triu(association_fc_significance*association_nonzero_sorted, k = 1)), 'o', color = (0,0.5,1), markersize = 4, markeredgewidth = 0, alpha = 0.8)
    plt.xlim(-2.4,2.4)
    # plt.ylim(-0.2, 7)
    ax.hlines(-np.log10(0.05/n_hypothesis), -2, 2, linestyles = '--', color = (1,0.5,0), linewidth = 1)
    ax.vlines(-1, 0, 8, linestyles = '--', color = (1,0.5,0), linewidth = 1)
    ax.vlines(1, 0, 8, linestyles = '--', color = (1,0.5,0), linewidth = 1)
    for i in range(len(labels_list_sorted)):
        pos_x = positions_list_sorted[i][0]
        pos_y = positions_list_sorted[i][1]
    ...
    for i in range(len(labels_list)):
        ax.text(-0.6, -i*10, '{}. {}'.format(i+1, labels_list_sorted[i]), color = theme_color, fontsize = 6)

    plt.subplots_adjust(left=0.22, right=0.98, top=0.98, bottom=0.05, hspace = 0.6)
    plt.ylim(-25,10)
    plt.xlim(-0.1,2.6)
    plt.axis('off')
    ...
    # significant differential spatial association presentation
    theme_color = 'white'
    font_size = 12
    positions_list_sorted = [positions_list[k] for k in np.argsort(fc_list)[::-1]]
    labels_list_sorted = [labels_list[k] for k in np.argsort(fc_list)[::-1]]
    ha_list_sorted = [ha_list[k] for k in np.argsort(fc_list)[::-1]]
    fig = plt.figure()
    fig.set_size_inches(cm_to_inches(8.5), cm_to_inches(11))
    gs = GridSpec(3,1)
    ax = plt.subplot(gs[0:2,0])
    plt.plot(np.log2(np.triu(association_fc*association_nonzero_sorted, k = 1)), -np.log10(np.triu(association_fc_significance*association_nonzero_sorted, k = 1)), 'o', color = (0,0.5,1), markersize = 4, markeredgewidth = 0, alpha = 0.8)
    plt.xlim(-2.4,2.4)
    ...
    for i in range(len(labels_list_sorted)):
        pos_x = positions_list_sorted[i][0]
        pos_y = positions_list_sorted[i][1]
        ax.text(pos_x, pos_y, i+1, ha = ha_list_sorted[i], va = 'center', fontsize = font_size, color = theme_color)

    ax.tick_params(direction = 'in', labelsize = font_size, colors = theme_color)
    ...
    plt.xlabel('$\log_{2}$(FC)', fontsize = font_size, color = theme_color)
    plt.ylabel('-$\log_{10}$(p)', fontsize = font_size, color = theme_color)
    ax = plt.subplot(gs[2,0])
    for i in range(len(labels_list)):
        ax.text(-0.6, -i*10, '{}. {}'.format(i+1, labels_list_sorted[i]), color = theme_color, fontsize = font_size)

    plt.subplots_adjust(left=0.22, right=0.98, top=0.98, bottom=0.05, hspace = 0.6)
    plt.ylim(-25,10)
    plt.xlim(-0.1,2.6)
    plt.axis('off')
    plt.savefig('{}/association_fold_change_significance_labeled_presentation.svg'.format(data_folder), dpi = 300, transparent = True)
    plt.close()
    return
...
def main():
    ...
    epithelial_region_filename = '{}_epithelial_region.npy'.format(sample)
    epithelial_region = np.load(epithelial_region_filename)
    ...
    cell_info_filtered, image_identification_filtered = remove_epithepial_objects_and_debris(image_seg, image_identification, adjacency_seg, epithelial_region, taxon_lookup, cell_info, sample)
